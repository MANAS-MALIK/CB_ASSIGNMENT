...
def refresh_pm_status_and_notify():
    ...
    schedules = frappe.get_all('PM Schedule', filters={'status': ['in', ['Scheduled', 'Due', 'Overdue']]}, fields=['name', 'status', 'next_due', 'assigned_technician', 'task', 'outlet'])
    newly_overdue = []
    for s in schedules:
        if not s.next_due:
            continue
...
def _notify_overdue(schedule):
    ...
    recipients = set()
    if tech:
        user, manager = frappe.db.get_value('Maintenance Technician', tech, ['user', 'reports_to']) or (None, None)
        if user:
            recipients.add(user)
        if manager:
    ...
    for user in recipients:
        notification = frappe.get_doc({'doctype': 'Notification Log', 'subject': subject, 'for_user': user, 'type': 'Alert', 'document_type': 'PM Schedule', 'document_name': schedule.get('name')})
