...
class PMTemplate(Document):
	...
	def roll_out_to_outlet(self, outlet, start_date=None):
		"""Roll this template onto an outlet.

		Creates (or reuses) the Store Asset for this category at the outlet, then
		creates one PM Schedule per template task. This is the "define once, roll
		across stores" primitive - no per-store task re-entry.
		"""
		start_date = getdate(start_date or nowdate())

		asset_name = frappe.db.get_value(
			"Store Asset",
			{"outlet": outlet, "asset_category": self.asset_category},
			"name",
		)
		if not asset_name:
		...
		for task in self.tasks:
			exists = frappe.db.exists(
				"PM Schedule",
				{"store_asset": asset_name, "task": task.task},
			)
			if exists:
				continue
			frappe.get_doc(
				{
					...
					"next_due": next_due_from(start_date, task.frequency),
				}
			).insert()
			created += 1
...
def roll_out_to_all_outlets(template, start_date=None):
	"""Roll a template onto every outlet whose city the template should serve.

	v1 keeps it simple: roll onto all outlets. Scope can later be limited by
	city/zonal office without changing the template.
	"""
