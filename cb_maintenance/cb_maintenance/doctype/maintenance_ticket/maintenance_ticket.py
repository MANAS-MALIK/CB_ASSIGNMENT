...
class MaintenanceTicket(Document):
    ...
    def set_zonal_office(self):
        if self.outlet and (not self.zonal_office):
            self.zonal_office = frappe.db.get_value('Maintenance Outlet', self.outlet, 'zonal_office')
    ...
    def suggest_spare_part(self):
        ...
        parts = frappe.get_all('Spare Part', filters={'match_keyword': ['is', 'set']}, fields=['name', 'match_keyword', 'asset_category'])
        matches = [p for p in parts if p.match_keyword and p.match_keyword.lower() in text]
        if not matches:
            return
    ...
    def notify_assignee(self):
        if not self.assigned_technician:
            return
        user = frappe.db.get_value('Maintenance Technician', self.assigned_technician, 'user')
        if not user:
            return
        from frappe.desk.form.assign_to import add as assign_add
        try:
            assign_add({'assign_to': [user], 'doctype': self.doctype, 'name': self.name, 'description': self.subject})
